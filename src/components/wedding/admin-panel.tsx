    ...
    return (
      ...
      >
        ...
        >
          <button
            onClick={onClose}
            className="absolute top-3 right-3 sm:top-4 sm:right-4 text-amber-700/60 font-semibold hover:text-amber-700 p-1"
          >
          ...
          <p className="text-stone-700/70 font-display text-xl sm:text-2xl italic mb-6 sm:mb-8 px-2">
            Ingresa la contraseña para administrar
          </p>

          <div className="space-y-4 text-left">
            <div>
              <Label htmlFor="pwd" className="text-amber-700/80 font-semibold tracking-widest uppercase text-[16px] sm:text-2xl">
                Contraseña
              </Label>
              <Input
                id="pwd"
                ...
                placeholder="••••••••"
                className="mt-2 bg-white/80 border-amber-600/30 text-stone-800 placeholder:text-stone-400 font-display text-xl sm:text-2xl tracking-widest h-11"
                autoFocus
              />
            </div>
            ...
            <Button
              onClick={verify}
              disabled={verifying}
              className="w-full bg-gradient-to-r from-amber-700 via-amber-500 to-amber-700 hover:from-amber-600 hover:via-amber-400 hover:to-amber-500 text-white font-medium tracking-widest uppercase rounded-full h-12 sm:py-6 text-xl sm:text-2xl"
            >
              {verifying ? (
                <>
      ...
      <div className="max-w-6xl mx-auto p-3 sm:p-4 md:p-8">
        ...
        <div className="flex items-center justify-between mb-4 sm:mb-6 gap-2">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-stone-700/60 font-display text-xl sm:text-2xl mt-1">
          ...
          <Button
            variant="outline"
            onClick={onClose}
            className="border-amber-600/40 text-amber-700 hover:bg-amber-400/10 h-9 sm:h-10 px-2 sm:px-3 text-xl sm:text-2xl shrink-0"
          >
        ...
        <div className="flex gap-2 mb-6 sm:mb-8 border-b border-amber-600/20">
          <button
            ...
            className={`flex items-center gap-2 px-3 sm:px-6 py-2 sm:py-3 font-display text-xl sm:text-2xl border-b-2 transition-all ${
              activeTab === "rsvps"
                ? "border-amber-600 text-amber-700"
                : "border-transparent text-stone-700/60 font-medium hover:text-stone-800"
            }`}
          >
            <Users className="w-4 h-4 sm:w-5 sm:h-5" />
            <span>Confirmaciones</span>
            {rsvps.length > 0 && (
              <span className="ml-1 bg-amber-600/20 text-amber-700 text-xl px-1.5 py-0.5 rounded-full">
                {rsvps.length}
              </span>
            )}
          </button>
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-6 sm:mb-8">
              <div className="glass-card rounded-lg sm:rounded-2xl p-2.5 sm:p-5 text-center border-amber-600/30">
                ...
                <div className="text-[15px] sm:text-2xl uppercase tracking-widest text-stone-700/60 font-medium mt-1">
                  Confirmados
                </div>
              </div>
            ...
            {rsvps.length > 0 && (
              <div className="flex justify-end mb-4">
                <Button
                  variant="outline"
                  onClick={exportCSV}
                  className="border-amber-600/40 text-amber-700 hover:bg-amber-400/10 h-9 sm:h-10 text-xl sm:text-2xl"
                >
            ...
            {loadingRsvps ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                {/* Attending */}
                <div>
                  <h2 className="text-xl sm:text-2xl font-serif text-amber-700 mb-3 sm:mb-4 flex items-center gap-2">
                    <Check className="w-4 h-4 sm:w-5 sm:h-5" />
                    Confirmados ({attending.length})
                  </h2>
                  <div className="space-y-2 sm:space-y-3 max-h-[60vh] overflow-y-auto pr-1 sm:pr-2">
                    {attending.length === 0 ? (
                      <p className="text-stone-700/40 text-xl italic">Aún nadie ha confirmado</p>
                    ) : (
                      attending.map((r) => (
                        <div
                          key={r.id}
                        ...
                        >
                          <div className="flex items-start justify-between gap-2">
                            <div className="min-w-0">
                              <div className="font-display text-xl sm:text-2xl text-stone-800 truncate">{r.name}</div>
                              <div className="text-xl sm:text-2xl text-amber-700/80 font-semibold">
                                {r.guests} {r.guests === 1 ? "invitado" : "invitados"}
                              </div>
                            </div>
                            <div className="text-[16px] sm:text-2xl text-stone-700/40 shrink-0">
                              {new Date(r.createdAt).toLocaleDateString("es-MX")}
                            </div>
                          </div>
                          {r.message && (
                            <p className="mt-2 text-xl sm:text-2xl text-stone-700/70 italic font-display border-l-2 border-amber-600/30 pl-2 sm:pl-3 break-words">
                              {r.message}
                            </p>
                          )}
                        </div>
                ...
                {/* Not attending */}
                <div>
                  <h2 className="text-xl sm:text-2xl font-serif text-stone-800 mb-3 sm:mb-4 flex items-center gap-2">
                    <X className="w-4 h-4 sm:w-5 sm:h-5" />
                    No asistirán ({notAttending.length})
                  </h2>
                  <div className="space-y-2 sm:space-y-3 max-h-[60vh] overflow-y-auto pr-1 sm:pr-2">
                    {notAttending.length === 0 ? (
                      <p className="text-stone-700/40 text-xl italic">Nadie ha declinado aún</p>
                    ) : (
                      notAttending.map((r) => (
                        <div
                          key={r.id}
                        ...
                        >
                          <div className="flex items-start justify-between gap-2">
                            <div className="font-display text-xl sm:text-2xl text-stone-800 truncate min-w-0 flex-1">{r.name}</div>
                            <div className="text-[16px] sm:text-2xl text-stone-700/40 shrink-0">
                              {new Date(r.createdAt).toLocaleDateString("es-MX")}
                            </div>
                          </div>
        ...
        {activeTab === "photos" && (
          <>
            ...
            <div className="glass-card rounded-xl sm:rounded-2xl p-4 sm:p-6 mb-6 border-amber-600/30">
              <h3 className="font-serif text-xl sm:text-2xl text-stone-800 mb-3 sm:mb-4 flex items-center gap-2">
              ...
              <div className="space-y-3 sm:space-y-4">
                <Input
                  ...
                  className="bg-white/70 border-amber-600/30 text-stone-800 file:bg-amber-500/20 file:text-amber-700 file:border-0 file:rounded file:px-2 file:py-1 file:mr-2 file:font-sans text-xl sm:text-2xl h-10 sm:h-11"
                />
                <Input
                  value={caption}
                  onChange={(e) => setCaption(e.target.value)}
                  placeholder="Descripción (opcional)"
                  className="bg-white/70 border-amber-600/30 text-stone-800 placeholder:text-stone-400 font-display text-xl sm:text-2xl h-10 sm:h-11"
                />
                <Button
                  onClick={handleUpload}
                  disabled={uploading || !file}
                  className="w-full bg-gradient-to-r from-amber-700 to-amber-800 hover:from-amber-600 hover:to-amber-700 text-white font-medium tracking-wider uppercase rounded-full h-11 text-xl sm:text-2xl"
                >
                  {uploading ? (
                    <>
                      <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                ...
                <p className="text-[16px] sm:text-2xl text-stone-700/50 text-center">
            ...
            {loadingPhotos ? (
              ...
              <div className="text-center py-12 glass-card rounded-xl sm:rounded-2xl">
                ...
                <p className="text-stone-700/40 text-xl sm:text-2xl mt-2">
                  ...
                  >
                    ...
                    {photo.caption && (
                      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-2">
                        <p className="text-white text-[16px] sm:text-2xl font-display italic line-clamp-2">
                          {photo.caption}
                        </p>
                      </div>
                    )}
        ...
        {activeTab === "settings" && (
          <>
            <div className="glass-card rounded-xl sm:rounded-2xl p-4 sm:p-6 border-amber-600/30">
              ...
              <p className="text-xl sm:text-2xl text-stone-700/60 mb-4 sm:mb-6 font-display italic">
              ...
              {!settingsForm ? (
                ...
                <div className="space-y-4 sm:space-y-5">
                  {/* Fecha y hora */}
                  <div>
                    <Label className="text-amber-700/80 font-semibold tracking-widest uppercase text-[16px] sm:text-2xl flex items-center gap-1.5">
                    ...
                    <Input
                      ...
                      className="mt-2 bg-white/70 border-amber-600/30 text-stone-800 font-display text-xl sm:text-2xl h-10 sm:h-11"
                    />
                    <p className="text-[16px] sm:text-2xl text-stone-700/50 mt-1">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      className="mt-2 bg-white/70 border-amber-600/30 text-stone-800 placeholder:text-stone-400 font-display text-xl sm:text-2xl h-10 sm:h-11"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      className="mt-2 bg-white/70 border-amber-600/30 text-stone-800 placeholder:text-stone-400 font-mono text-xl sm:text-2xl h-10 sm:h-11"
                    />
                    <p className="text-[16px] sm:text-2xl text-stone-700/50 mt-1">
          ...
          >
            ...
            >
              <div className="text-center">
                ...
                <p className="text-stone-700/70 text-xl mb-2 font-display">
                  Esta acción no se puede deshacer.
                </p>
                {deletingPhoto.caption && (
                  <p className="text-stone-600 text-xl italic mb-4 px-4">
                    "{deletingPhoto.caption}"
                  </p>
                )}
                {!deletingPhoto.caption && (
                ...
                <div className="flex gap-3">
                  <button
                    onClick={() => setDeletingPhoto(null)}
                    className="flex-1 px-4 py-3 rounded-full border border-stone-300 text-stone-700 text-xl font-medium uppercase tracking-wider hover:bg-stone-50 transition-colors"
                    type="button"
                  >
                    Cancelar
                  </button>
                  <button
                    ...
                    className="flex-1 px-4 py-3 rounded-full bg-red-600 hover:bg-red-700 text-white text-xl font-medium uppercase tracking-wider transition-colors flex items-center justify-center gap-2"
                    type="button"
                  >
