...
export function HeroSection() {
  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center px-4 py-20 text-center overflow-hidden">
      <FloralCorner className="absolute top-6 left-6" />
      <FloralCorner className="absolute top-6 right-6" flip />
      ...
      >
        ...
        <p className="mt-6 text-amber-200/70 tracking-[0.4em] uppercase text-xs sm:text-sm font-sans">
          {tagline}
        </p>
        ...
        <div className="my-8">
          <motion.h1
            ...
            className="text-5xl sm:text-7xl md:text-8xl font-serif italic text-gold-gradient leading-tight"
          >
            {bride.shortName}
          </motion.h1>
          <div className="my-2 sm:my-4 flex items-center justify-center gap-4">
            <span className="h-px w-12 bg-gradient-to-r from-transparent to-amber-400/60" />
            <span className="text-3xl sm:text-4xl font-serif text-amber-300">&</span>
        ...
        <p className="text-amber-100/80 text-sm sm:text-base tracking-widest uppercase font-sans">
          {formatDateLong(weddingDate)}
        </p>

        <DecorativeDivider className="mt-12" />

        <p className="max-w-md text-amber-50/70 text-base sm:text-lg font-display leading-relaxed italic">
        ...
        <a
          href="#rsvp"
          className="mt-12 inline-flex items-center justify-center rounded-full bg-gradient-to-r from-amber-600 via-amber-400 to-amber-600 px-8 py-3 text-sm font-medium uppercase tracking-widest text-black hover:scale-105 transition-transform shadow-lg shadow-amber-500/20"
        >
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 animate-bounce">
        <div className="text-amber-400/60 text-xs tracking-widest">▼</div>
      </div>
    </section>
  );
}
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-10">
          {items.map((item, i) => (
            <motion.div
              ...
              className="glass-card rounded-2xl p-6 flex items-center gap-4 hover:border-amber-400/50 transition-colors"
            >
              <div className="shrink-0 w-12 h-12 rounded-full bg-gradient-to-br from-amber-500/20 to-violet-600/20 flex items-center justify-center border border-amber-400/40">
              ...
              <div>
                ...
                <div className="text-lg font-display text-amber-50">
                  {item.value}
                </div>
              </div>
            </motion.div>
        ...
        {additionalInfo.giftNote && (
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.8 }}
            className="mt-10 glass-card rounded-2xl p-6 sm:p-8 flex items-start gap-4 max-w-2xl mx-auto"
          >
            <Gift className="shrink-0 w-6 h-6 text-amber-300 mt-1" />
            <div>
              <h3 className="font-serif text-xl text-amber-100 mb-2">Mesa de regalos</h3>
              <p className="text-amber-50/70 font-display text-lg leading-relaxed italic">
                {additionalInfo.giftNote}
              </p>
            </div>
          </motion.div>
    ...
    <section className="py-20 px-4 relative">
      <div className="max-w-4xl mx-auto">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          transition={{ duration: 0.8 }}
          className="glass-card rounded-3xl p-6 sm:p-10 mt-8"
        >
          <div className="text-center">
            <MapPin className="w-10 h-10 text-amber-400 mx-auto mb-4" />
            <h3 className="text-2xl sm:text-3xl font-serif text-amber-100">
              {venue.name}
            </h3>
            <p className="text-amber-50/70 font-display text-lg mt-2 max-w-md mx-auto italic">
              {venue.address}
            </p>
            ...
            <Button
              onClick={openMaps}
              size="lg"
              className="mt-8 bg-gradient-to-r from-amber-600 via-amber-400 to-amber-600 hover:from-amber-500 hover:via-amber-300 hover:to-amber-500 text-black font-medium tracking-wider uppercase rounded-full px-8 shadow-lg shadow-amber-500/20"
            >
          ...
          <div className="mt-8 rounded-2xl overflow-hidden border border-amber-400/20 aspect-video">
            <iframe
    ...
    <section className="py-20 px-4 relative">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="relative mt-12 pl-8 sm:pl-0">
          {/* Vertical line */}
          <div className="absolute left-3 sm:left-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-transparent via-amber-500/40 to-transparent" />
          ...
          {schedule.map((item, i) => (
            ...
            >
              {/* Dot */}
              <div
                className={`absolute top-2 w-3 h-3 rounded-full bg-amber-400 ring-4 ring-amber-400/20 ${
                  i % 2 === 0
                    ? "left-[-21px] sm:left-auto sm:right-[-7px]"
                    : "left-[-21px] sm:left-[-7px]"
                }`}
              />
              <div className="glass-card rounded-xl p-4">
                <div className="text-amber-300 font-serif text-2xl">{item.time}</div>
                <div className="text-amber-50 font-display text-lg mt-1">{item.title}</div>
                <div className="text-amber-100/60 text-sm mt-1">{item.description}</div>
              </div>
            </motion.div>
          ))}
        </div>
